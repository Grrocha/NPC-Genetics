...
def SendToDB(population, society):
    for index, row in population.iterrows():
        statement = "Insert Into tb_npc_list values ("+ str(society) + ",'UNNAMED'," + str(row['Race']) + "," + str(row['Gender']) + "," + str(row['GenderIdentity']) + "," + str(row['Personality']) + "," + str(row['Sexuality'])+ ")"
        print(statement)
        SqlConnector.conn.ExecuteTransaction("Insert Into tb_npc_list values ("+ str(society) + ",'UNNAMED'," + str(row['Race']) + "," + str(row['Gender']) + "," + str(row['GenderIdentity']) + "," + str(row['Personality']) + "," + str(row['Sexuality'])+ ")")
    

def CreateNewPopulation(possibilitiesArray, populationCap, society, limitedGenes = [], isLimited = False):
    societyIds = []
    ...
    SendToDB(popDf, society)
    return popDf
    
populationDF = CreateNewPopulation(possibilitiesArray, 10, 1);